...
import           Plutus.V2.Ledger.Api        (CurrencySymbol (CurrencySymbol),
                                              ...
                                              POSIXTime, Validator, ValidatorHash (ValidatorHash), Address (addressCredential, addressStakingCredential, Address))
...
credentialLedgerToPlutus (ScriptHashObj (ScriptHash h)) = Plutus.ScriptCredential $ ValidatorHash $ toBuiltin $ hashToBytes h
...
tryReadAddress :: String -> Maybe Plutus.V2.Ledger.Api.Address
tryReadAddress x = case Api.deserialiseAddress Api.AsAddressAny $ pack x of
    Nothing                                          -> Nothing
    Just (Api.AddressByron _)                        -> Nothing
    Just (Api.AddressShelley (ShelleyAddress _ p s)) -> Just Address
